import React from 'react';
import { Link } from 'react-router-dom';
import { ArrowRight, Code2, Layout, Smartphone, Mail } from 'lucide-react';
import { useLanguage } from '../context/LanguageContext';
import { useTheme } from '../context/ThemeContext';
import ImageCompany from '../image/logo.jpg';

const Home: React.FC = () => {
  const { t } = useLanguage();
  const { isDark } = useTheme();

  return (
    <div className="flex flex-col">
      {/* Hero Section */}
      <section className="min-h-[90vh] flex items-center py-20 relative overflow-hidden">
        {/* Abstract Background Element */}
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-blue-500/10 rounded-full blur-[100px] -z-10 animate-pulse-slow"></div>
        <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-purple-500/10 rounded-full blur-[100px] -z-10"></div>

        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          <div className="grid md:grid-cols-2 gap-8 lg:gap-16 items-center">
            
            {/* Left: Text Content */}
            <div className="text-center md:text-left z-10 order-2 md:order-1">
              <div className="opacity-0 animate-slide-up">
                 <div className={`inline-flex items-center px-3 py-1 mb-6 rounded-full border text-xs font-mono font-medium backdrop-blur-sm transition-colors ${
                   isDark 
                     ? 'bg-slate-900/50 border-blue-500/30 text-blue-300' 
                     : 'bg-white/50 border-blue-500/30 text-blue-600'
                 }`}>
                    <span className="w-2 h-2 bg-green-500 rounded-full mr-2 animate-pulse"></span>
                    {t.home.available}
                 </div>
              </div>

              <h1 className={`opacity-0 animate-slide-up-delay-1 text-5xl md:text-7xl font-bold tracking-tight mb-6 transition-colors ${
                isDark ? 'text-slate-100' : 'text-gray-900'
              }`}>
                {t.home.heroTitlePrefix} <br className="hidden md:block" />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400 font-mono">
                  {t.home.heroTitleSuffix}
                </span>
              </h1>
              
              <p className={`opacity-0 animate-slide-up-delay-2 text-lg md:text-xl mb-8 max-w-lg mx-auto md:mx-0 leading-relaxed transition-colors ${
                isDark ? 'text-slate-400' : 'text-gray-600'
              }`}>
                {t.home.intro}
              </p>
              
              <div className="opacity-0 animate-slide-up-delay-3 flex flex-col sm:flex-row justify-center md:justify-start gap-4">
                <Link
                  to="/experience"
                  className="inline-flex justify-center items-center px-8 py-3.5 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-500 hover:-translate-y-0.5 transition-all duration-300 shadow-lg shadow-blue-500/25 group font-mono"
                >
                  {t.home.btnExperience}
                  <ArrowRight className="ml-2 h-4 w-4 transition-transform group-hover:translate-x-1" />
                </Link>
                <Link
                  to="/projects"
                  className={`inline-flex justify-center items-center px-8 py-3.5 text-sm font-semibold border rounded-lg transition-all duration-300 font-mono group ${
                    isDark 
                      ? 'text-slate-300 bg-slate-900 border-slate-700 hover:bg-slate-800 hover:text-white hover:border-slate-600' 
                      : 'text-gray-700 bg-white border-gray-300 hover:bg-gray-50 hover:text-gray-900 hover:border-gray-400'
                  }`}
                >
                  {t.home.btnProjects}
                  <ArrowRight className="ml-2 h-4 w-4 transition-transform group-hover:translate-x-1" />
                </Link>
              </div>
            </div>

            {/* Right: Coding Image (Circular) */}
            <div className="opacity-0 animate-slide-up-delay-2 relative order-1 md:order-2 mb-12 md:mb-0 flex justify-center md:justify-end">
              <div className="relative w-64 h-64 md:w-80 md:h-80 lg:w-[28rem] lg:h-[28rem] aspect-square">
                {/* Circular Image Container */}
                <div className={`w-full h-full rounded-full overflow-hidden border-4 shadow-2xl shadow-blue-500/20 group transition-colors ${
                  isDark ? 'border-slate-800/80' : 'border-gray-300/80'
                }`}>
                  {/* Image Overlay Gradient */}
                  <div className={`absolute inset-0 bg-gradient-to-t via-transparent to-transparent z-10 ${
                    isDark ? 'from-slate-950/60' : 'from-white/60'
                  }`}></div>
                  
                  {/* The Image */}
                  <img 
                    src={ImageCompany} 
                    alt="Zumar coding on laptop" 
                    className="w-full h-full object-cover transform transition-transform duration-700 group-hover:scale-110"
                  />
                </div>

                {/* Floating Badge - Positioned Outside at Bottom Edge */}
                <div className={`absolute -bottom-4 left-1/2 transform -translate-x-1/2 z-30 backdrop-blur-md border-2 py-2.5 px-5 md:px-7 rounded-full flex items-center shadow-2xl shadow-blue-500/30 whitespace-nowrap hover:scale-105 hover:border-blue-500/50 transition-all duration-300 cursor-default ${
                  isDark 
                    ? 'bg-slate-900/95 border-slate-700' 
                    : 'bg-white/95 border-gray-300'
                }`}>
                  <Code2 className="w-4 h-4 mr-2 text-blue-400" />
                  <span className={`text-sm font-semibold mr-2 transition-colors ${
                    isDark ? 'text-slate-100' : 'text-gray-900'
                  }`}>{t.home.codingBadge}</span>
                </div>
              </div>
              
              {/* Decorative elements behind image */}
              <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full h-full max-w-[300px] md:max-w-[400px] bg-gradient-to-tr from-blue-500/20 to-purple-500/20 rounded-full blur-2xl -z-10"></div>
            </div>

          </div>
        </div>
      </section>

      {/* Skills / Philosophy */}
      <section className={`py-24 border-t transition-colors duration-300 ${
        isDark 
          ? 'bg-slate-900/30 border-slate-800/60' 
          : 'bg-gray-100/50 border-gray-200/60'
      }`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            <div className={`group p-8 rounded-2xl border transition-all duration-300 hover:shadow-2xl hover:shadow-blue-500/10 ${
              isDark 
                ? 'bg-slate-900 hover:bg-slate-800/80 border-slate-800 hover:border-blue-500/30' 
                : 'bg-white hover:bg-gray-50 border-gray-200 hover:border-blue-500/30'
            }`}>
              <div className="w-12 h-12 bg-blue-500/10 text-blue-400 rounded-lg flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300 border border-blue-500/20">
                <Code2 className="h-6 w-6" />
              </div>
              <h3 className={`text-xl font-bold mb-3 font-mono transition-colors ${
                isDark ? 'text-slate-100' : 'text-gray-900'
              }`}>{t.home.cards.clean.title}</h3>
              <p className={`leading-relaxed transition-colors ${
                isDark ? 'text-slate-400' : 'text-gray-600'
              }`}>
                {t.home.cards.clean.desc}
              </p>
            </div>
            
            <div className={`group p-8 rounded-2xl border transition-all duration-300 hover:shadow-2xl hover:shadow-cyan-500/10 ${
              isDark 
                ? 'bg-slate-900 hover:bg-slate-800/80 border-slate-800 hover:border-cyan-500/30' 
                : 'bg-white hover:bg-gray-50 border-gray-200 hover:border-cyan-500/30'
            }`}>
              <div className="w-12 h-12 bg-cyan-500/10 text-cyan-400 rounded-lg flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300 border border-cyan-500/20">
                <Smartphone className="h-6 w-6" />
              </div>
              <h3 className={`text-xl font-bold mb-3 font-mono transition-colors ${
                isDark ? 'text-slate-100' : 'text-gray-900'
              }`}>{t.home.cards.responsive.title}</h3>
              <p className={`leading-relaxed transition-colors ${
                isDark ? 'text-slate-400' : 'text-gray-600'
              }`}>
                {t.home.cards.responsive.desc}
              </p>
            </div>
            
            <div className={`group p-8 rounded-2xl border transition-all duration-300 hover:shadow-2xl hover:shadow-purple-500/10 ${
              isDark 
                ? 'bg-slate-900 hover:bg-slate-800/80 border-slate-800 hover:border-purple-500/30' 
                : 'bg-white hover:bg-gray-50 border-gray-200 hover:border-purple-500/30'
            }`}>
              <div className="w-12 h-12 bg-purple-500/10 text-purple-400 rounded-lg flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300 border border-purple-500/20">
                <Layout className="h-6 w-6" />
              </div>
              <h3 className={`text-xl font-bold mb-3 font-mono transition-colors ${
                isDark ? 'text-slate-100' : 'text-gray-900'
              }`}>{t.home.cards.ux.title}</h3>
              <p className={`leading-relaxed transition-colors ${
                isDark ? 'text-slate-400' : 'text-gray-600'
              }`}>
                {t.home.cards.ux.desc}
              </p>
            </div>
          </div>
        </div>
      </section>

      {/* Contact Section */}
      <section className={`py-16 border-t transition-colors duration-300 ${
        isDark 
          ? 'bg-slate-950/50 border-slate-800/60' 
          : 'bg-gray-50/50 border-gray-200/60'
      }`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="max-w-2xl mx-auto">
            <h2 className={`text-3xl md:text-4xl font-bold mb-4 font-mono transition-colors ${
              isDark ? 'text-slate-100' : 'text-gray-900'
            }`}>
              Mari Bekerja Sama
            </h2>
            <p className={`text-lg mb-8 transition-colors ${
              isDark ? 'text-slate-400' : 'text-gray-600'
            }`}>
              Tertarik untuk berkolaborasi? Saya selalu terbuka untuk proyek menarik dan peluang baru.
            </p>
            <a
              href="mailto:buildapptech@gmail.com"
              className="inline-flex justify-center items-center px-8 py-4 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-500 hover:-translate-y-0.5 transition-all duration-300 shadow-lg shadow-blue-500/25 group font-mono"
            >
              <Mail className="mr-2 h-5 w-5" />
              buildapptech@gmail.com
            </a>
          </div>
        </div>
      </section>
    </div>
  );
};

export default Home;